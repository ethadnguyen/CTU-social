...
const MessagePage = () => {
  ...
  const handleFriendClick = async (friendId) => {
    let existingConversation = conversations.find(conv => conv.recipients.includes(friendId));

    if (existingConversation) {
      if (!existingConversation.recipients.includes(id)) {
        try {
          const response = await axiosInstance.put(`/chat/conversation/${existingConversation._id}`, {
            userId: id,
          });
          existingConversation = response.data.conversation;
          setUserConversations(prev =>
            prev.map(conv => conv._id === existingConversation._id ? existingConversation : conv)
          );
        } catch (error) {
